...
export function CatalogFilters() {
  ...
  return <aside className="rounded-2xl border border-[#e6ddd8] bg-white p-5 lg:sticky lg:top-24 lg:self-start" aria-label="Bộ lọc sản phẩm">
    ...
    <fieldset className="mt-6"><legend className="font-semibold">Kích thước</legend><div className="mt-3 flex flex-wrap gap-2">{sizes.map((size) => <button type="button" className={`min-h-10 rounded-lg border px-3 text-sm ${current.get("size") === size ? "border-[#ce7a85] bg-[#fcf4f5]" : "border-[#d9d0cb]"}`} onClick={() => set("size", current.get("size") === size ? "" : size)} key={size}>{size}</button>)}</div></fieldset>
    <button className="mt-6 text-sm text-[#ce7a85] underline" onClick={() => router.push("/products")}>Xóa bộ lọc</button>
  </aside>;
}
