    ...
    <section>
      ...
      <div className="divide-y divide-[#e4dad5]">{cart.items.map((item) => {
        ...
        return <article className="grid grid-cols-[24px_92px_1fr] gap-4 py-6 sm:grid-cols-[24px_120px_1fr_auto]" key={item.id}>
          ...
          <div className="col-start-3 text-right sm:col-start-auto"><strong>{money(item.lineTotal)}</strong><button className="mt-3 block text-sm text-[#713c33] underline" disabled={pendingId === item.id} onClick={() => void mutate(item.id, () => apiRequest(`/api/cart?itemId=${encodeURIComponent(item.id)}`, { method: "DELETE" }))}>Xóa</button></div>
        </article>;
      })}</div>
    </section>
