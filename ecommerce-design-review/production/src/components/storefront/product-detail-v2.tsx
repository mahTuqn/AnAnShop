...
export function ProductDetailV2({ product }: { product: Product }) {
  ...
  return <div className="grid gap-10 lg:grid-cols-[1.1fr_.9fr]" data-testid="product-detail"><section aria-label="Ảnh sản phẩm"><div className="relative aspect-[4/5] overflow-hidden rounded-3xl bg-[#faeff1]"><img className="size-full object-cover" src={product.images[image] ?? product.image} alt={`${product.name}${image ? ` – ảnh ${image + 1}` : ""}`}/>{product.images.length > 1 && <><button type="button" onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 flex size-10 items-center justify-center rounded-full bg-white/80 text-black shadow-sm hover:bg-white transition-colors" aria-label="Ảnh trước">❮</button><button type="button" onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 flex size-10 items-center justify-center rounded-full bg-white/80 text-black shadow-sm hover:bg-white transition-colors" aria-label="Ảnh tiếp theo">❯</button></>}</div>{product.images.length > 1 && <div className="mt-3 flex gap-3" role="group" aria-label="Chọn ảnh sản phẩm">{product.images.map((src, index) => <button type="button" className={`size-20 overflow-hidden rounded-xl border-2 ${image === index ? "border-[#ce7a85]" : "border-transparent"}`} onClick={() => setImage(index)} aria-label={`Xem ảnh ${index + 1}`} aria-pressed={image === index} key={src}><img className="size-full object-cover" src={src} alt=""/></button>)}</div>}</section>
    <section className="lg:sticky lg:top-24 lg:self-start"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#c26b77]">{product.categoryLabel}</p><h1 className="mt-3 font-serif text-4xl leading-tight">{product.name}</h1><p className="mt-4 text-sm text-[#625853]" aria-label={`${product.rating} trên 5 sao, ${product.reviewCount} đánh giá`}><span aria-hidden>★</span> {product.rating} · {product.reviewCount} đánh giá</p><div className="mt-5 flex gap-3 text-xl"><strong>{money(product.price)}</strong>{product.compareAtPrice && <del className="text-base text-[#766c67]">{money(product.compareAtPrice)}</del>}</div><p className="mt-6 leading-7 text-[#615454]">{product.description}</p>
      <fieldset className="mt-8"><legend className="font-semibold">Màu sắc · <span className="font-normal">{color}</span></legend><div className="mt-3 flex gap-3">{product.colors.map((item) => <button type="button" className={`size-11 rounded-full border-4 ${color === item.name ? "ring-2 ring-[#ce7a85] ring-offset-2" : ""}`} style={{ background: item.hex }} onClick={() => setColor(item.name)} aria-label={`Màu ${item.name}`} aria-pressed={color === item.name} key={item.name}/>)}</div></fieldset>
      <fieldset className="mt-8" id="chon-size"><legend className="font-semibold">Kích thước</legend><div className="mt-3 grid grid-cols-4 gap-2">{product.sizes.map((item) => <button type="button" className={`min-h-13 rounded-xl border text-sm ${size === item.name ? "border-[#ce7a85] bg-[#fcf4f5]" : "border-[#cbbfba]"}`} onClick={() => { setSize(item.name); setError(""); }} aria-pressed={size === item.name} data-testid={`size-${item.name}`} key={item.name}>{item.name}</button>)}</div></fieldset>
      <fieldset className="mt-8"><legend className="font-semibold">Số lượng</legend>
        <div className="mt-3 flex h-13 w-32 items-center rounded-xl border border-[#cbbfba] bg-white">
          <button type="button" className="flex h-full w-10 items-center justify-center text-xl text-[#ce7a85] hover:bg-[#fcf4f5] rounded-l-xl transition-colors" onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Giảm số lượng">-</button>
          ...
          <button type="button" className="flex h-full w-10 items-center justify-center text-xl text-[#ce7a85] hover:bg-[#fcf4f5] rounded-r-xl transition-colors" onClick={() => setQuantity(quantity + 1)} aria-label="Tăng số lượng">+</button>
        </div>
      </fieldset>
      ...
      <div className="mt-7 grid gap-3 sm:grid-cols-2"><Button disabled={pending !== null} data-testid="add-to-cart" variant="secondary" onClick={() => add("cart")}>{pending === "cart" ? "Đang thêm…" : "Thêm vào giỏ"}</Button><Button disabled={pending !== null} className="w-full" onClick={() => add("buy")}>{pending === "buy" ? "Đang chuẩn bị…" : "Mua ngay"}</Button></div><ul className="mt-7 grid gap-3 border-y border-[#f2e4e6] py-5 text-sm text-[#615454]"><li>✓ Giao hàng dự kiến trong 2–4 ngày</li><li>✓ Miễn phí vận chuyển từ 699.000đ</li><li>✓ Hỗ trợ đổi size trong 14 ngày</li></ul></section></div>;
}
