...
export function AuthFormIntegrated({ mode }: { mode: Mode }) {
  ...
  return <div data-testid={`auth-${mode}-page`}><p className="text-xs font-semibold tracking-[.2em] text-[#c26b77]">TÀI KHOẢN AN AN</p><h1 className="mt-3 font-serif text-4xl">{config[0]}</h1><p className="mt-3 leading-7 text-[#6b5e5e]">{config[1]}</p><form className="mt-8 space-y-5" onSubmit={submit} data-testid="auth-form">{mode === "register" && <Input label="Họ và tên" name="name" autoComplete="name"/>}<Input label="Email" name="email" type="email" autoComplete="email"/>{(mode === "login" || mode === "register") && <Input label="Mật khẩu" name="password" type="password" minLength={8} autoComplete={mode === "login" ? "current-password" : "new-password"}/>} {mode === "register" && <Input label="Xác nhận mật khẩu" name="confirmPassword" type="password" minLength={8} autoComplete="new-password"/>}{mode === "reset" && <><Input label="Mật khẩu mới" name="password" type="password" minLength={8}/><Input label="Nhập lại mật khẩu" name="confirmPassword" type="password" minLength={8}/></>}{mode === "register" && <label className="flex items-start gap-3 text-sm"><input className="mt-1" type="checkbox" required/><span>Tôi đồng ý với điều khoản sử dụng và chính sách bảo mật.</span></label>}{mode === "login" && <div className="text-right"><a className="text-sm text-[#ce7a85] underline" href="/forgot-password">Quên mật khẩu?</a></div>}<Button className="w-full" disabled={pending} data-testid="auth-submit">{pending ? "Đang xử lý..." : mode === "login" ? "Đăng nhập" : mode === "register" ? "Tạo tài khoản" : mode === "forgot" ? "Gửi hướng dẫn" : "Lưu mật khẩu mới"}</Button>{error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-800" role="alert">{error}</p>}{status && <p className="rounded-xl bg-white p-3 text-sm text-[#536b57]" role="status">{status}</p>}</form><p className="mt-7 text-center text-sm text-[#6b5e5e]">{mode === "login" ? <>Chưa có tài khoản? <a className="font-semibold text-[#ce7a85] underline" href="/register">Đăng ký ngay</a></> : <>Đã có tài khoản? <a className="font-semibold text-[#ce7a85] underline" href="/login">Đăng nhập</a></>}</p></div>;
}
