...
type ApiOrder = { id: string; code: string; status: string; paymentStatus: string; paymentMethod: string; subtotal: number; shippingFee: number; discountTotal: number; grandTotal: number; placedAt: string; address: { fullName: string; phone: string; province: string; district: string; ward: string; line1: string }; items: { variantId: string; productId: string; productSlug?: string; productName: string; variantName: string; quantity: number; lineTotal: number }[]; returns?: { id: string; status: string; reason: string; adminNote?: string | null; requestedAt: string; wasAccepted?: boolean }[] };
...
export function AccountOrderDetailClient({ id }: { id: string }) {
  ...
  if (error && !order) return <section><Link className="text-sm text-[#713c33] underline" href="/account/orders">← Quay lại đơn hàng</Link><p className="mt-5 rounded-xl bg-red-50 p-4 text-sm text-red-800" role="alert">{error}</p></section>;
  if (!order) return <PageSkeleton/>;
  return <section><Link className="text-sm text-[#713c33] underline" href="/account/orders">← Quay lại đơn hàng</Link><div className="mt-5 flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm text-[#625853]">Đặt ngày {new Intl.DateTimeFormat("vi-VN").format(new Date(order.placedAt))}</p><h2 className="mt-1 font-serif text-3xl">Đơn #{order.code}</h2></div><span className="rounded-full bg-[#edf3ed] px-3 py-1 text-sm text-[#356646]">{order.status}</span></div>{message && <p className="mt-5 rounded-xl bg-green-50 p-4 text-sm text-green-800" role="status">{message}</p>}{error && <p className="mt-5 rounded-xl bg-red-50 p-4 text-sm text-red-800" role="alert">{error}</p>}<div className="mt-5 flex flex-wrap gap-3"><button className="rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50" disabled={pending} onClick={() => void act("REBUY")}>Mua lại</button>{["PENDING", "CONFIRMED"].includes(order.status) && <button className="rounded-xl border border-red-700 px-4 py-2 text-sm font-semibold text-red-700 disabled:opacity-50" disabled={pending} onClick={() => void act("CANCEL")}>Hủy đơn</button>}{order.status === "DELIVERED" && <button className="rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50" disabled={pending} onClick={() => void act("RETURN")}>Yêu cầu đổi/trả</button>}</div>
