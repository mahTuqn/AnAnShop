...
export function CheckoutClientV2() {
  ...
  if (!cart.items.length) return <section className="mx-auto max-w-xl rounded-3xl bg-white p-8 text-center shadow-sm" data-testid="checkout-empty"><h1 className="font-serif text-3xl">Giỏ hàng chưa có sản phẩm</h1><p className="mt-3 text-[#6b5e5e]">{loadError || "Hãy thêm sản phẩm trước khi bắt đầu thanh toán."}</p><ButtonLink href="/products" className="mt-6">Khám phá sản phẩm</ButtonLink></section>;

  return <div className="grid gap-10 lg:grid-cols-[1fr_380px]" data-testid="checkout-page"><div><ol className="mb-10 grid grid-cols-3 gap-2" aria-label="Tiến trình thanh toán">{["Thông tin", "Vận chuyển", "Thanh toán"].map((label, index) => <li className={`border-t-2 pt-3 text-sm ${step >= index + 1 ? "border-[#ce7a85] font-semibold text-[#ce7a85]" : "border-[#d9d0cb] text-[#706763]"}`} aria-current={step === index + 1 ? "step" : undefined} key={label}>{index + 1}. {label}</li>)}</ol>
