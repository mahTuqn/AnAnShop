...
export function P0OperationsPage({ kind }: { kind: OperationKind }) {
  ...
  return <section aria-labelledby={`${kind}-title`} data-testid={`admin-${kind}-page`}>
    ...
    {selected && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4"><section role="dialog" aria-modal="true" aria-label="Chi tiết vận hành" className="w-full max-w-lg rounded-2xl bg-white p-6"><div className="flex justify-between gap-4"><div><p className="text-xs uppercase tracking-wider text-rose-700">Chi tiết</p><h2 className="mt-1 text-xl font-semibold">{selected.primary}</h2></div><button type="button" onClick={() => setSelected(null)} aria-label="Đóng chi tiết" className="h-11 w-11 rounded-xl border border-slate-200 text-xl">×</button></div><dl className="mt-6 divide-y divide-slate-100 rounded-xl border border-slate-200"><div className="p-4"><dt className="text-xs text-slate-500">Thông tin</dt><dd className="mt-1 font-medium">{selected.secondary}</dd></div><div className="p-4"><dt className="text-xs text-slate-500">Phạm vi / giá trị</dt><dd className="mt-1 font-medium">{selected.meta}</dd></div><div className="p-4"><dt className="text-xs text-slate-500">Trạng thái</dt><dd className="mt-1 font-medium">{selected.status}</dd></div></dl><button type="button" className="mt-6 min-h-11 rounded-xl bg-[#b06b75] px-5 text-sm font-semibold text-white">Cập nhật</button></section></div>}
