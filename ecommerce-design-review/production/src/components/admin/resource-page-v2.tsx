...
export function AdminResourcePageV2({ resource, resourceKey }: { resource: AdminResource; resourceKey: string }) {
  ...
  return <section aria-labelledby={`${resourceKey}-title`} data-testid={`admin-${resourceKey}-page`}>
    <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-1 text-xs font-semibold uppercase tracking-[0.2em] text-rose-700">An An Backoffice</p><h1 id={`${resourceKey}-title`} className="text-2xl font-semibold sm:text-3xl">{resource.title}</h1><p className="mt-2 text-sm text-slate-600">{resource.description}</p></div>{resource.createLabel && <button type="button" onClick={() => setCreating(true)} className="min-h-11 rounded-xl bg-[#b06b75] px-5 text-sm font-semibold text-white" data-testid={`${resourceKey}-create`}>+ {resource.createLabel}</button>}</div>
    ...
    <AdminAsyncState empty={!shown.length}><div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr>{resource.headers.map((header) => <th scope="col" key={header} className="px-5 py-3">{header}</th>)}<th scope="col" className="px-5 py-3">Trạng thái</th><th scope="col"><span className="sr-only">Thao tác</span></th></tr></thead><tbody className="divide-y divide-slate-100">{shown.map((item) => <tr key={item.id}>{item.cells.map((cell, index) => <td key={`${item.id}-${index}`} className="px-5 py-4">{resourceKey === "products" && index === 0 ? <ProductEditorLink id={item.id} label={cell} /> : cell}</td>)}<td className="px-5 py-4"><span className="rounded-full bg-rose-50 px-2.5 py-1 text-xs font-medium text-rose-800">{item.status}</span></td><td className="px-5 py-4 text-right"><button type="button" onClick={() => setSelected(item)} className="min-h-10 rounded-lg px-3 font-semibold text-rose-800" aria-label={`Xem chi tiết ${item.cells[0]}`}>Chi tiết</button></td></tr>)}</tbody></table></div></AdminAsyncState>
