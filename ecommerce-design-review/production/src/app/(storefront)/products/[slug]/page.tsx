...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6"><nav className="mb-8 text-sm text-[#776b65]" aria-label="Đường dẫn">Trang chủ / {product.categoryLabel} / <span className="text-[#332824]">{product.name}</span></nav><ProductDetail product={product}/><ProductReviews productId={product.id}/><section className="mt-24"><h2 className="font-serif text-3xl">Có thể mẹ cũng thích</h2><div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4">{related.map((item) => <ProductCard product={item} key={item.id}/>)}</div></section></main>;
}
